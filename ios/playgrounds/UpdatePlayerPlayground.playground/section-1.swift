            ...
            if httpResponse.statusCode == 200 {
                ...
                if data != nil {
                    ...
                    error: nil) as? NSDictionary{
                        ...
                        // signed in and has a result
                        
                        if let user_rec = jsonResult["user"] as? NSDictionary{
                            
                            println(user_rec["id"])
                            
                            var inner_request = NSMutableURLRequest(URL: NSURL(string: "http://localhost:8888/u/update/score")!)
                            var inner_params = Dictionary<String, AnyObject>()
                            inner_params["score_increment"] = 2
                            inner_request.HTTPShouldHandleCookies = true
                            inner_request.HTTPMethod = "POST"
                            inner_request.HTTPBody = stringFromParameters(inner_params).dataUsingEncoding(NSUTF8StringEncoding)
